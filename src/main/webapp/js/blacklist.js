 ...
 */
Ext.namespace('IsmpHB', 'IsmpHB.blacklist');
...
var nodeCodeData = [ [ 'GD', '全省' ], [ 'GZ', '广州' ], [ 'SG', '韶关' ],
		[ 'SZ', '深圳' ], [ 'ZH', '珠海' ], [ 'ST', '汕头' ], [ 'FS', '佛山' ],
		[ 'JM', '江门' ], [ 'ZJ', '湛江' ], [ 'MM', '茂名' ], [ 'ZQ', '肇庆' ],
		[ 'HZ', '惠州' ], [ 'MZ', '梅州' ], [ 'SW', '汕尾' ], [ 'HY', '河源' ],
		[ 'YJ', '阳江' ], [ 'QY', '清远' ], [ 'DG', '东莞' ], [ 'ZS', '中山' ],
		[ 'CZ', '潮州' ], [ 'JY', '揭阳' ], [ 'YF', '云浮' ] ];

var sourceData = [ [ 1, '省IT黑名单' ], [ 2, '声讯业务黑名单' ], [ 3, '地市黑名单' ],
		[ 4, '公务纳费' ], [ 5, '局公话免费' ], [ 6, '市场公免' ], [ 7, '红名单'] ];

var rangeTypeData = [ [ 1, '区域' ], [ 2, '业务' ] ];

IsmpHB.blacklist.ItemForm = Ext.extend(Ext.form.FormPanel,
		{
			labelWidth : 90,
			labelAlign : 'right',
			border : false,
			...
			rangeTypeField : new Ext.form.ComboBox({
				...
				listeners : {
					select : function(combo, record, index) {
						var id = Ext.getCmp('rangeTypeField_f').getValue();
			...
			constructor : function(config) {
				...
				IsmpHB.blacklist.ItemForm.superclass.constructor.apply(this,
						arguments);
			},
...
IsmpHB.blacklist.ItemDlg = Ext.extend(Ext.Window, {
	title : '新增特殊号码',
	layout : 'fit',
	modal : true,
	width : 500,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		this.configForm = new IsmpHB.blacklist.ItemForm({});
		this.configForm.nodeCodeField.setValue('GD');
		config.items.push(this.configForm);
		IsmpHB.blacklist.ItemDlg.superclass.constructor.apply(this, arguments);
	},
	toAdd : function() {
...
var createDlg = new IsmpHB.blacklist.ItemDlg({});
...
IsmpHB.blacklist.Blacklist = Ext.extend(Ext.grid.GridPanel, {
	loadMask : {
		msg : '正在加载数据，请稍候...'
	},
	title : '计费特殊号码查询',
	...
	constructor : function(config) {
		...
		var a = IsmpHB.common.getPermission('7-1');
...
//			config.tbar.push(this.editBtn);
		...
		this.addBtn.on('click', function() {
			if(IsmpHB.common.getSession("loginInfo").type==16){
					createDlg.configForm.sourceField.setValue(3);
					createDlg.configForm.sourceField.disable();
					var a = IsmpHB.common.getSession("loginInfo").nodeCode;
		...
		createDlg.configForm.commitBtn.on('click', function() {
			if (createDlg.configForm.getForm().isValid()) {
				...
				var rangeType = createDlg.configForm.rangeTypeField.getValue();
				var productId = createDlg.configForm.productIdField.getValue();
				var source = createDlg.configForm.sourceField.getValue();
				var memo = createDlg.configForm.memoField.getValue();
				var nodeCode = createDlg.configForm.nodeCodeField.getValue();
				var areaNo = areaNoData[nodeCode];
//				if(IsmpHB.common.getSession("loginInfo").type==16&&source==1){
...
//				}
				...
				if (tel == "" || rangeType == "" || source == ""
					...
					createDlg.configForm.rangeTypeField.setValue(rangeType);
					createDlg.configForm.productIdField.setValue(productId);
					createDlg.configForm.sourceField.setValue(source);
					createDlg.configForm.memoField.setValue(memo);
					createDlg.configForm.nodeCodeField.setValue(nodeCode);
				} else {
					var req = {
						url : IsmpHB.req.BLACK_ADD,
						params : {
						...
						callback : function(o) {
							if (o.success == "true") {
								...
								createDlg.configForm.rangeTypeField.reset();
								createDlg.configForm.productIdField.reset();
								createDlg.configForm.sourceField.reset();
								createDlg.configForm.memoField.reset();
								createDlg.configForm.nodeCodeField.reset();
							} else {
