 ...
 * @author pansenxin
 */
Ext.namespace('LBSReader', 'LBSReader.ecrmOrderUser');
LBSReader.ecrmOrderUser.basicItemForm = Ext.extend(Ext.form.FormPanel, {
			title : '基本信息',
			labelWidth : 100,
			labelAlign : 'right',
			border : false,
			...
			constructor : function(config) {
				...
				LBSReader.ecrmOrderUser.basicItemForm.superclass.constructor
						.apply(this, arguments);

			},
			setValue : function(o) {
				if (null != o[0]) {
					this.cityIdField.setValue(LBSReader.renderer
							.ECRMPRODUCTCITYCOBO(o[0]));
				}
				if (null != o[1]) {
				...
				if (null != o[4]) {
					this.stateField.setValue(LBSReader.renderer
							.ECRMORDERUSERSTATE(o[4]));
				}
				if (null != o[5]) {
...
LBSReader.ecrmOrderUser.productItemForm = Ext.extend(Ext.Panel, {
			title : '产品属性',
...
LBSReader.ecrmOrderUser.productItemDlg = Ext.extend(Ext.Window, {
	autoScroll : true,
	modal : true,
	width : 1000,
	height : 331,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		this.basicconfigForm = new LBSReader.ecrmOrderUser.basicItemForm({});
		this.productconfigForm = new LBSReader.ecrmOrderUser.productItemForm({});
		config.items.push([this.basicconfigForm, this.productconfigForm]);
		LBSReader.ecrmOrderUser.productItemDlg.superclass.constructor.apply(
				this, arguments);
	},
	showProduct : function(data) {
		this.basicconfigForm.setValue(data);
		var req = {
			url : LBSReader.req.ECRM_ORDERUSER_PRODUCT,
			params : {
			...
			scope : this,
			callback : function(o) {
				this.productconfigForm.body.update(o.ecrmProductString)
			}
		};
		LBSReader.Ajax.send(req);
	}
});
LBSReader.ecrmOrderUser.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '订购关系',
	autoScroll : true,
	...
	constructor : function(config) {
		this.productdlg = new LBSReader.ecrmOrderUser.productItemDlg({});
		...
		var a = LBSReader.common.getPermission('10-5');
		var arr = [];
		...
		config.tbar.push(new Ext.Panel({
					...
					items : [{
						xtype : 'toolbar',
						border : false,
						items : ['客户ID：', this.custIdField, '客户名称：',
								this.custNameField, '产品类型：', this.proTypeField,
								'企业总机类型：', this.isvpnField, '地市：',
								this.cityIdField, '服务状态：', this.stateField]
					}, {
						xtype : 'toolbar',
						border : false,
						items : ['服务编码：', this.servNbrField, '产品接入码：',
								this.accNbrField, '创建时间：',
		...
		this.cm = new Ext.grid.ColumnModel([this.sm, {
					...
					menuDisabled : true,
					dataIndex : 'cityId',
					renderer : LBSReader.renderer.ECRMPRODUCTCITYCOBO,
					width : 100
				}, {
					header : '客户ID',
					align : 'center',
					...
					menuDisabled : true,
					dataIndex : 'state',
					renderer : LBSReader.renderer.ECRMORDERUSERSTATE,
					width : 100
				}, {
					header : '产品类型',
					align : 'center',
		...
		LBSReader.ecrmOrderUser.DataGrid.superclass.constructor.apply(this,
				arguments);
	...
	searchItems : function(name) {
		var dateValid = LBSReader.customFunctions.dateValid(
				this.createDateField.getValue(), this.updateDateField
						.getValue());
		if (!dateValid) {
