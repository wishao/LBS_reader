 ...
 * @author johnny0086
 */
Ext.namespace('IsmpHB','IsmpHB.actions');

IsmpHB.actions.draw = function(container){
		...
		paper.path(["M",s.getBBox().x+20,s.getBBox().y+40,"L",c.getBBox().x+50,c.getBBox().y].join(" ")).attr({
			stroke: 'green',
			'stroke-width': 3
		});
		...
		var path = ["M",c.getBBox().x+50,c.getBBox().y+40,"L",r.getBBox().x+50,r.getBBox().y];
