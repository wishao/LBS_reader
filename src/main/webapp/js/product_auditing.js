 ...
 */
Ext.namespace('LBSReader', 'LBSReader.auditing');
LBSReader.auditing.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '产品审核',
	autoScroll : true,
	...
	viewConfig : {
		getRowClass : function(record, rowIndex, rowParams, store) {
			if (LBSReader.data.AUDITING_TYPE.PASS == record.data.auditingFlag
					|| LBSReader.data.AUDITING_TYPE.AUTO == record.data.auditingFlag) {
				return 'combo_status_normal';
			} else if (LBSReader.data.AUDITING_TYPE.UNPASS == record.data.auditingFlag) {
	...
	flagCombo : new Ext.form.ComboBox({
				...
				valueField : 'id',
				width : 120,
				store : LBSReader.store.AUDITING_TYPE,
				value : LBSReader.data.AUDITING_TYPE.ALL,
				listeners : {
					'collapse' : function() {
	...
	constructor : function(config) {
		...
		var a = LBSReader.common.getPermission('1-3');
		...
		this.cm = new Ext.grid.ColumnModel([this.sm, {
					...
					dataIndex : 'status',
					width : 60,
					renderer : LBSReader.renderer.STATUS
				}, {
					header : '计费周期',
					align : 'center',
					menuDisabled : true,
					...
					width : 80,
					renderer : LBSReader.renderer.OPERATETYPE
				}, {
					header : '业务资费',
					align : 'center',
					menuDisabled : true,
					...
					renderer : function(obj) {
						return obj / LBSReader.data.RATE;
					}
					...
					width : 80,
					renderer : LBSReader.renderer.AUDITING_FLAG
				}, {
					header : '审核者',
					align : 'center',
					menuDisabled : true,
		...
		LBSReader.auditing.DataGrid.superclass.constructor.apply(this,
				arguments);
	...
	loadItems : function() {
		...
		if (LBSReader.data.AUDITING_TYPE.ALL != this.flagCombo.getValue()) {
			this.getStore().baseParams = {
				method : 'search',
				auditingType : this.flagCombo.getValue()
			}
	...
	removeItem : function() {
		...
		if (LBSReader.data.AUDITING_TYPE.PASS == r.data.auditingFlag
				|| LBSReader.data.AUDITING_TYPE.AUTO == r.data.auditingFlag
				|| LBSReader.data.AUDITING_TYPE.UNPASS == r.data.auditingFlag) {
		...
		Ext.MessageBox.confirm("提示", "确认要删除所选记录吗？", function(id) {
					if (id == "yes") {
						var req = {
							url : LBSReader.req.PRODUCT_AUDITING_REV,
							params : {
								method : 'remove',
								id : r.data.id
							},
	...
	passItem : function() {
		...
		if (this.validateCyle(r.data.chargingCycle)) {
			if (LBSReader.data.AUDITING_TYPE.PASS == r.data.auditingFlag
					|| LBSReader.data.OPERATE_TYPE.ADD == r.data.operateType) {
		...
		if (LBSReader.data.AUDITING_TYPE.UNPASS == r.data.auditingFlag) {
		...
		Ext.MessageBox.confirm("提示", "确认要通过所选记录的审核吗？", function(id) {
			if (id == "yes") {
				var req = {
					url : LBSReader.req.PRODUCT_AUDITING_PASS,
					params : {
						method : 'pass',
						operator : LBSReader.common.getSession('loginInfo').name,
	...
	unPassItem : function() {
		...
		Ext.MessageBox.confirm("提示", "确认要不通过所选记录的审核吗？", function(id) {
			if (id == "yes") {
				var req = {
					url : LBSReader.req.PRODUCT_AUDITING_UNPASS,
					params : {
						method : 'unPass',
						operator : LBSReader.common.getSession('loginInfo').name,
						updateTime : new Date().valueOf(),
	...
	validateCyle : function(value) {
		if (LBSReader.data.CHARGING_CYCLE.TIME == value
				|| LBSReader.data.CHARGING_CYCLE.MINUTE == value
				|| LBSReader.data.CHARGING_CYCLE.SECOND == value) {
			return true;
		}
		return false;
	},
