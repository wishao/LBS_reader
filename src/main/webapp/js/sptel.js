 ...
 */
Ext.namespace('IsmpHB', 'IsmpHB.sptel');

IsmpHB.sptel.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : 'CP/SP信息管理',
	autoScroll : true,
	...
	constructor : function(config) {
		...
		this.adDlg = new IsmpHB.sptel.SpDlg({});
		var config = config || {};
		config.tbar = config.tbar || [];
		var a = IsmpHB.common.getPermission('6-1');
		...
		this.cm = new Ext.grid.ColumnModel([ this.sm, {
            ...
            width : 200,
            renderer : IsmpHB.renderer.SP_STATUS
        }
		]);
		IsmpHB.sptel.DataGrid.superclass.constructor.apply(this, arguments);
		...
		this.syncBtn.on('click', function() {
		    ...
		    if(rs.data.vsopStatus==0){
                Ext.MessageBox.confirm("提示", "该SP已经同步到VSOP平台，确认再次同步吗？", function(id) {
                    if (id == "yes") {
                        this.syncItems();
                    }
                },this);
            }else{
				Ext.MessageBox.confirm('提示', '确定要同步给VSOP平台！',function(btn){
					if(btn == 'yes')
						this.syncItems();
				}, this);
            }
			...
			var req = {
				...
				callback : function(o) {
					if (true == o.success || 'true' == o.success) {
							var req = {
				                url : IsmpHB.req.SERVICEPROVINFO_MGR,
				                params : {
				                    spCode : r.data.spCode,
				                    method : 'deleteSp'
				                },
				                ...
				                callback : function(o) {
				                    if (true == o.success || 'true' == o.success) {
				                        Ext.MessageBox.alert('提示', '成功删除该sp厂家！', null, this);
				                        this.pagingbar.doRefresh();
				                        //this.getStore().load();
				                    }else{
				                        Ext.MessageBox.alert('提示', '删除该sp厂家失败！', null, this);
                                        this.pagingbar.doRefresh();
                                        //this.getStore().load();
				                    }
				                }
					       ...
					       Ext.MessageBox.alert('提示', '该sp厂家有关联产品不允许删除！', null, this);
                           this.getStore().load();
					}
	...
	searchItems : function() {
		...
		if(this.nameField.getValue().trim()=='' && this.nameField.getValue().length>1){
...
IsmpHB.sptel.SpForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 70,
	labelAlign : 'right',
	...
	constructor : function(config) {
		...
		IsmpHB.sptel.SpForm.superclass.constructor.apply(this, arguments);
		this.addBtn.on('click', function() {
		    if (this.isEDIT) {
                this.toEdit();
            } else {
...
IsmpHB.sptel.SpDlg = Ext.extend(Ext.Window, {
	layout : 'fit',
	modal : true,
	width : 350,
	height : 330,
	...
	constructor : function(config) {
		var config = config || {};
		config.items = this.items || [];
		this.configForm = new IsmpHB.sptel.SpForm({});
		config.items.push(this.configForm);
		IsmpHB.sptel.SpDlg.superclass.constructor.apply(this, arguments);
