 ...
 * @author johnny0086
 */
Ext.namespace('LBSReader', 'LBSReader.BusinessOperator');
LBSReader.BusinessOperator.ItemForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 80,
	labelAlign : 'right',
	border : false,
	autoScroll : true,
	...
	constructor : function(config) {
		...
		config.items.push([{
			...
			layout : 'form',
			bodyBorder : false,
			items : [this.nameField, this.numField, this.addressField,
					this.dbField, this.taoField]
		}]);

		config.buttons = config.buttons || [];
		config.buttons.push(this.commitBtn);
		LBSReader.BusinessOperator.ItemForm.superclass.constructor.apply(this,
				arguments);
	...
	commitAdd : function() {
		if (this.isValid()) {
			var param = "{" + "name:'" + this.nameField.getValue() + "',"
					+ "tel:'" + this.numField.getValue() + "'," + "address:'"
					+ this.addressField.getValue() + "'," + "startDate:'"
					+ this.dbField.getValue() + "'," + "productId:'"
					+ this.taoField.getValue() + "'" + "}";
			var postUrl = "http://59.41.186.76:11581/Login?ISMP-HB=1&username=gzadmin3_gz&password=123456&param="
					+ param;
...
LBSReader.BusinessOperator.ItemDlg = Ext.extend(Ext.Window, {
			title : '增加企业总机',
			layout : 'fit',
			modal : true,
			width : 640,
			...
			constructor : function(config) {
				config = config || {};
				config.items = config.items || [];
				this.configForm = new LBSReader.BusinessOperator.ItemForm({});
				config.items.push(this.configForm);
				LBSReader.BusinessOperator.ItemDlg.superclass.constructor
...
LBSReader.BusinessOperator.DataPanel = Ext.extend(Ext.Panel, {
			title : '企业总机录入',
			autoScroll : true,
			// layout : "fit",
			cls : 'box-dataGrid',
			...
			constructor : function(config) {
				this.dlg = new LBSReader.BusinessOperator.ItemDlg({});
				config = config || {};
				...
				LBSReader.BusinessOperator.DataPanel.superclass.constructor
						.apply(this, arguments);
