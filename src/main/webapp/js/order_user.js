 ...
 * @author johnny0086
 */
Ext.namespace('LBSReader', 'LBSReader.orderuser');

LBSReader.orderuser.ProductGrid = Ext.extend(Ext.grid.GridPanel, {
	autoScroll : true,
	height : 165,
	...
	constructor : function(config) {
		...
		LBSReader.orderuser.ProductGrid.superclass.constructor.apply(this,
				arguments);
		/**
...
LBSReader.orderuser.ItemForm = Ext.extend(Ext.form.FormPanel, {
			labelWidth : 70,
			labelAlign : 'right',
			border : false,
			...
			constructor : function(config) {
				this.isEDIT = false;
				this.grid = new LBSReader.orderuser.ProductGrid({});
				config = config || {};
				...
				LBSReader.orderuser.ItemForm.superclass.constructor.apply(this,
						arguments);
...
LBSReader.orderuser.ItemDlg = Ext.extend(Ext.Window, {
			title : '用户订购管理',
			layout : 'fit',
			modal : true,
			width : 350,
			...
			constructor : function(config) {
				config = config || {};
				config.items = config.items || [];
				this.configForm = new LBSReader.orderuser.ItemForm({});
				config.items.push(this.configForm);
				LBSReader.orderuser.ItemDlg.superclass.constructor.apply(this,
						arguments);
...
LBSReader.orderuser.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '包月类订购管理',
	autoScroll : true,
	...
	telField : new Ext.form.TextField({
				...
				width : 120,
				/*
				 * regex : new RegExp('^([0-9]{10,12})$'), regexText :
				 * '所填写的电话号码不合法',
				 */
				allowBlank : false
			}),
	...
	constructor : function(config) {
		this.dlg = new LBSReader.orderuser.ItemDlg({});
		...
		var a = LBSReader.common.getPermission('2-1');
		...
		LBSReader.orderuser.DataGrid.superclass.constructor.apply(this,
				arguments);
		...
		this.packageCombo.on('expand', function(combo) {
					combo.getStore().each(function(r) {
						if (r.data.chargingCycle == 3
								|| r.data.chargingCycle == 4
								|| r.data.chargingCycle == 5) {
		...
		this.addBtn.on('click', function() {
					...
					Ext.StoreMgr.get('addEffectPackageWithoutPage').removeAll();
					order_user_ProductAttr.removeAll();
					LBSReader.orderuser.productAddDlg.show();
					order_user_submitButton.buttons[0].setDisabled(true);
					order_user_chargeType.setDisabled(true);
					order_user_flagCombo.reset();
					order_user_orderTel.reset();
					order_user_orderName.reset();
					order_user_dbField.reset();
					order_user_ddField.reset();
					order_user_packageNameField.reset();
				}, this);
	...
	rmvItems : function() {
		...
		var req = {
			url : LBSReader.req.COMBO_CANCEL,
			params : {
				timestamp : new Date().valueOf(),
			...
			callback : function(o) {
				if (o.success) {
					...
					if (o.message === null) {
						Ext.MessageBox.alert('提示', '对不起，处理失败！请稍后重试。',
								function() {
								}, this);
					} else {
...
// 填充产品包信息弹出框内容（弹左下弹）
LBSReader.orderuser.showAddView = function(grid, rowIndex, colIndex) {
	var r = Ext.StoreMgr.get('addEffectPackageWithoutPage').getAt(rowIndex);
	order_user_productAddForm.get('order_package_id').setValue(r.data.id);
	order_user_productAddForm.get('order_package_name').setValue(r.data.name);
	order_user_productAddForm.get('order_package_code').setValue(r.data.code);
	order_user_productAddForm.get('order_package_pairValue')
			.setValue(r.data.pairValue);
	order_user_productAddForm.get('order_package_billFlag')
			.setValue(LBSReader.renderer.CHARGINGMODE(r.data.billFlag));
	order_user_productAddForm.get('order_package_chargingCode')
			.setValue(r.data.chargingCode);
	order_user_productAddForm.get('order_package_chargingDesc')
			.setValue(r.data.chargingDesc);
	order_user_productAddForm.get('order_package_chargingCycle')
			.setValue(LBSReader.renderer.CHARGINGCYCLE(r.data.chargingCycle));
	order_user_productAddForm.get('order_package_effectMode')
			.setValue(LBSReader.renderer.EFFECTMODE(r.data.effectMode));
	order_user_productAddForm.get('order_package_withDrawMode')
			.setValue(LBSReader.renderer.EFFECTMODE(r.data.withDrawMode));
	order_user_productAddForm.get('order_package_trialType')
			.setValue(LBSReader.renderer.TRIAL_TYPE(r.data.trialType));
	order_user_productAddForm.get('order_package_trialTerm')
			.setValue(r.data.trialTerm);
	order_user_productAddForm.get('order_package_useFlag')
			.setValue(LBSReader.renderer.VIEW_STATUS_FLAG(r.data.useFlag));
	order_user_productAddForm.get('order_package_beginCharg')
			.setValue(LBSReader.renderer.BEGIN_RULE(r.data.beginCharg));
	order_user_productAddForm.get('order_package_endPreCharg')
			.setValue(LBSReader.renderer.END_PRE_CHARG(r.data.endPreCharg));
	order_user_productAddForm.get('order_package_endAfterCharg')
			.setValue(LBSReader.renderer.END_AFTER_CHARG(r.data.endAfterCharg));
	LBSReader.orderuser.packageDlg.show();
}

// 新增订购框搜索产品包（弹左下）
LBSReader.orderuser.AddProductSearchPanel = Ext.extend(Ext.grid.GridPanel, {
	title : '产品包列表',
	autoScroll : true,
	store : Ext.StoreMgr.get('addEffectPackageWithoutPage'),
	order_user_packageNameField : new Ext.form.TextField({
		emptyText : '请填写产品包名称',
		maxLength : 80,
		msgTarget : 'side',
		width : 180,
		listeners : {
			specialkey : function(field, e) {
				if (e.getKey() == Ext.EventObject.ENTER) {
					...
					Ext.StoreMgr.get('addEffectPackageWithoutPage').removeAll();
					order_user_ProductAttr.removeAll();
					Ext.StoreMgr.get('addEffectPackageWithoutPage').load({
								params : {
									method : 'effectPackageNoPage',
									start : 0,
	...
	constructor : function(config) {
		config = config || {};
		order_user_packageNameField = this.order_user_packageNameField;
		config.tbar = config.tbar || [];
		config.tbar.push('类型：', this.flagCombo);
		config.tbar.push('名称：', this.order_user_packageNameField);
		config.tbar.push('->');
		...
		this.cm = new Ext.grid.ColumnModel([{
					...
					items : [{
						...
						tooltip : '查看产品包详细信息',
						handler : LBSReader.orderuser.showAddView
								.createDelegate(this)
					}]
				}, {
					xtype : 'actioncolumn',
					...
					items : [{
						getClass : function(html, meta, rec) {
							if (rec.get("useFlag") == LBSReader.data.STATUS_FLAG.EFFECT
									&& rec.get("chargingCycle") != LBSReader.data.CHARGING_CYCLE.TIME
									&& rec.get("chargingCycle") != LBSReader.data.CHARGING_CYCLE.MINUTE
									&& rec.get("chargingCycle") != LBSReader.data.CHARGING_CYCLE.SECOND) {
						...
						handler : LBSReader.orderuser.showProduct
								.createDelegate(this)

					}]
				}]);
		LBSReader.orderuser.AddProductSearchPanel.superclass.constructor.apply(
				this, arguments);
		// 查询产品包事件
		this.addSearchBtn.on('click', function() {
					...
					Ext.StoreMgr.get('addEffectPackageWithoutPage').load({
								params : {
									method : 'effectPackageNoPage',
									name : this.order_user_packageNameField
											.getValue(),
									packageType : '0',
									useFlag : '0'
								},
...
LBSReader.orderuser.AddProductAttrPanel = Ext.extend(Ext.Panel, {
			title : '产品附加信息',
			autoScroll : true,
			formBox : null,
			...
			constructor : function(config) {
				...
				order_user_ProductAttr = this.formBox;
				config.items.push(this.formBox);
				LBSReader.orderuser.AddProductAttrPanel.superclass.constructor
...
// 新增订购框产品包中的产品（弹右上）
LBSReader.orderuser.AddProductPanel = Ext.extend(Ext.grid.GridPanel, {
	title : '产品列表',
	autoScroll : true,
	...
	constructor : function(config) {
		...
		LBSReader.orderuser.AddProductPanel.superclass.constructor.apply(this,
				arguments);
		this.getSelectionModel().on('rowselect',
				function(sm, rowIndex, record) {
					var id = record.data.id;
		...
		Ext.StoreMgr.get('productSearchattr').load({
					...
					callback : function(r, o, s) {
						...
						if (null != r && 0 < r.length) {
							...
							order_user_ProductAttr.doLayout();
						}
						order_user_submitButton.buttons[0].setDisabled(false);
...
// 产品包信息框里面的选择跳转的方法
LBSReader.orderuser.showProduct = function(grid, rowIndex, colIndex) {
	Ext.StoreMgr.get('searchPackage').removeAll();
	var r = Ext.StoreMgr.get('addEffectPackageWithoutPage').getAt(rowIndex);
	...
	if (r.data.useFlag == LBSReader.data.STATUS_FLAG.EFFECT
			&& r.data.chargingCycle != LBSReader.data.CHARGING_CYCLE.TIME
			&& r.data.chargingCycle != LBSReader.data.CHARGING_CYCLE.MINUTE
			&& r.data.chargingCycle != LBSReader.data.CHARGING_CYCLE.SECOND) {
		...
		Ext.StoreMgr.get('searchPackage').load({
			...
			callback : function(r, o, s) {
				...
				order_user_ProductSM.selectRecords(cs);
			},
			scope : this
		});
...
// 详细产品包信息弹出框（弹左下弹）
LBSReader.orderuser.packageDlg = new Ext.Window({
			title : '产品包信息',
			layout : 'fit',
			modal : true,
			width : 450,
			...
			constrainHeader : true,
			closeAction : 'hide',
			items : [order_user_productAddForm = new Ext.form.FormPanel({
						labelWidth : 140,
						labelAlign : 'right',
						border : false,
						bodyStyle : 'padding:5px 10px 5px 10px;',
						...
						buttons : [{
									...
									handler : function() {// 点击取消按钮的操作事件
										LBSReader.orderuser.packageDlg.hide();
									}
								}]
					})]
		});
...
// 新增订购信息弹出框（弹）
LBSReader.orderuser.productAddDlg = new Ext.Window({
	title : '新增订购',
	layout : 'fit',
	modal : true,
	width : 960,
		...
		items : [new Ext.form.FormPanel({
			...
			items : [{
				...
				items : [{
							columnWidth : .5,
							layout : "form",
							items : [order_user_orderTel = new Ext.form.TextField(
									{
										fieldLabel : '客户电话',
										editable : false,
										mode : 'local',
							...
							columnWidth : .5,
							layout : "form",
							items : [order_user_orderName = new Ext.form.TextField(
									{
										fieldLabel : '客户姓名',
										editable : false,
										mode : 'local',
				...
				items : [{
					columnWidth : .5,
					layout : "form",
					items : [order_user_chargeType = new Ext.form.ComboBox({
								fieldLabel : '付费方式',
								editable : false,
								mode : 'local',
								triggerAction : 'all',
					...
					columnWidth : .5,
					layout : "form",
					items : [order_user_flagCombo = new Ext.form.ComboBox({
								fieldLabel : '试用标识',
								editable : false,
								mode : 'local',
								triggerAction : 'all',
				...
				items : [{
							columnWidth : .5,
							layout : "form",
							items : [order_user_dbField = new Ext.form.DateField(
									{
										fieldLabel : '生效日期',
										format : 'Y-m-d',
										allowBlank : false,
										emptyText : '有效期开始日期',
										msgTarget : 'side',
										name : 'order_user_dbField',
										id : 'order_user_dbField',
										width : 180,
										value : new Date
									})]
						}, {
							columnWidth : .5,
							layout : "form",
							items : [order_user_ddField = new Ext.form.DateField(
									{
										fieldLabel : '失效日期',
										format : 'Y-m-d',
										allowBlank : false,
										emptyText : '有效期结束日期',
										msgTarget : 'side',
										name : 'order_user_ddField',
										id : 'order_user_ddField',
										width : 180,
										value : new Date().add(Date.YEAR, 1)
									})]
						}]
			}]
		}), order_user_submitButton = new Ext.Panel({// border布局
			region : 'south',
			border : false,
			header : false,
			buttonAlign : 'center',
			buttons : [{
				...
				// 点击提交按钮的操作事件
				handler : function() {
					if (order_user_orderTel.getValue().trim().length < 1) {
						Ext.Msg.alert('提示', '请输入要正确的电话号码！');
						return;
					}
					var pattern2 = /^[0-9]+$/;
					if (!pattern2.exec(order_user_orderTel.getValue())) {
						Ext.Msg.alert('提示', '电话号码不能包含数字以外的字符！');
						return;
					}
					var pattern3 = /^[0-9]{10,12}$/;
					if (!pattern3.exec(order_user_orderTel.getValue().trim())) {
						Ext.Msg.alert('提示', '所填写的电话号码长度不符合要求，请检查！');
						return;
					}
					var dateValid = LBSReader.customFunctions.dateValid(
							order_user_dbField.getValue(), order_user_ddField
									.getValue());
					if (!dateValid) {
					...
					for (var i = 0; i < order_user_ProductAttr.items.length; i++) {
						if (order_user_ProductAttr
								.get(order_user_ProductAttr.items.items[i])
								.getValue() == ''
								&& !order_user_ProductAttr
										.get(order_user_ProductAttr.items.items[i]).allowBlank) {
							Ext.MessageBox
									.alert(
											'提示',
											order_user_ProductAttr
													.get(order_user_ProductAttr.items.items[i]).fieldLabel
													+ '不能为空！', null, this);
							return;
						}
					}
					var ps = [];
					var rs = order_user_ProductSM.getSelections();
					// 获取已选择的
					var attr = [];
					for (var i = 0; i < order_user_ProductAttr.items.length; i++) {
						var object = {};
						object['k'] = order_user_ProductAttr.items.items[i].name;
						object['v'] = order_user_ProductAttr
								.get(order_user_ProductAttr.items.items[i])
								.getValue();
						attr.push(object);
					}
					...
					var cromb = {
						Start_Time : order_user_dbField.getValue()
								.format(Date.patterns.ISO8601Middle),
						End_Time : order_user_ddField.getValue()
								.format(Date.patterns.ISO8601Middle),
						Status : '1',//
						usage : order_user_flagCombo.getValue(),
						product_list : ps
					};
					var req = {
						...
						params : {
							timestamp : new Date().valueOf(),
							tel : order_user_orderTel.getValue(),
							name : order_user_orderName.getValue(),
							hbPackageId : hbPackageId,
							hbPackageName : hbPackageName,
							hbPackageCode : hbPackageCode,
							chargeType : order_user_chargeType.getValue(),
							data : Ext.encode(cromb)
						},
						scope : this,
						callback : function(o) {
							if (o.success) {
										...
										function(e) {
											if (e == 'yes') {
												Ext.StoreMgr
														...
														.removeAll();
												Ext.StoreMgr
														.get('addEffectPackageWithoutPage')
														.removeAll();
												Ext.StoreMgr
														.get('addEffectPackageWithoutPage')
														.load({
												...
												order_user_ProductAttr
														.removeAll();
												LBSReader.orderuser.productAddDlg
														.show();
												order_user_submitButton.buttons[0]
														.setDisabled(true);
												order_user_chargeType
														.setDisabled(true);
												order_user_flagCombo.reset();
												order_user_dbField.reset();
												order_user_ddField.reset();
												order_user_packageNameField
														.reset();
											} else {
												LBSReader.orderuser.productAddDlg
														.hide();
								...
								if (o.message === null) {
									Ext.MessageBox.alert('提示', '添加失败！', null,
											this);
								} else {
			...
			items : [new Ext.Panel({// border布局
				...
				items : [new Ext.Panel({// border布局
					layout : 'border',
					border : false,
					items : [new LBSReader.orderuser.AddProductPanel({
										region : 'north',
										height : 200
									}), new Ext.Panel({
								region : 'center',
								...
								items : [new LBSReader.orderuser.AddProductAttrPanel(
										{})]
							})]
				})]
			}), new LBSReader.orderuser.AddProductSearchPanel({
						region : 'center'
					})]
		})]
	})]
