 ...
 * @author johnny0086
 */
Ext.namespace('IsmpHB', 'IsmpHB.batchOrderUser');
IsmpHB.batchOrderUser.DataPanel = Ext
		.extend(
				Ext.Panel,
				{
					title : '批量订购管理',
					...
					constructor : function(config) {
						...
						this.batchOrderUser = new IsmpHB.batchOrderUser.FileImportForm(
								{});
						...
						IsmpHB.crmInfo.DataPanel.superclass.constructor.apply(
								this, arguments);
					},
					getNotice : function() {
...
IsmpHB.batchOrderUser.FileImportForm = Ext
		.extend(
				Ext.form.FormPanel,
				{
					labelWidth : 100,
					...
					constructor : function(config) {
						...
						var a = IsmpHB.common.getPermission('2-3');
						var arr = [];
						...
						IsmpHB.batchOrderUser.FileImportForm.superclass.constructor
								.apply(this, arguments);
						this.fileSelectField.on('fileselected', function() {
							var fileName = this.fileSelectField.getValue();
							if (!fileName.match(/.xlsx?$/)) {
								Ext.MessageBox.alert('提示', '只支持导入Excel文件！',
										this.resetFileField(), this);
								return;
							}
						}, this);
						...
						this.commitBtn.on('click', function() {
							...
							if (operFlag == 0) {
								// 批量新增
								url = IsmpHB.req.ORDER_USER_FILE_UPLOAD;
							} else {
								// 批量拆机
								url = IsmpHB.req.ORDER_USER_FILE_UPLOAD_REMOVE;
							}
							var fileName = this.fileSelectField.getValue();
							if (fileName.length == 0) {
								Ext.MessageBox.alert("提示",
										'您未选择文件，请先选择要导入的Excel文件！');
								return;
							}
						...
						this.getForm().submit(
								{
									...
									success : function(form, o) {
										Ext.MessageBox.alert('Excel文件处理结果',
												o.result.message, function() {
													// this.ownerCt.hide();
												}, this);
									},
									failure : function(form, o) {
										Ext.MessageBox.alert('Excel文件处理结果',
												o.result.message, null, this);
									}
								});
					},
