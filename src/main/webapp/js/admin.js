 ...
 */
Ext.namespace('IsmpHB', 'IsmpHB.admin');
IsmpHB.admin.MyCheckboxGroup = Ext.extend(Ext.form.CheckboxGroup,{
    columns:2,
    dataUrl:'', //数据地址
    labelFiled:'name',
    valueFiled:'id',
    ...
    onRender:function(ct, position){ 
       ...
       //alert(items.length);
        ...
        IsmpHB.admin.MyCheckboxGroup.superclass.onRender.call(this, ct, position);
    }
});
Ext.reg('mycheckgroup',IsmpHB.admin.MyCheckboxGroup);


IsmpHB.admin.ModPwdForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 70,
	labelAlign : 'right',
	...
	constructor : function(config) {
		...
		IsmpHB.admin.ModPwdForm.superclass.constructor.apply(this, arguments);
	...
	toAdd : function(){
		...
		var req = {
			...
			callback : function(o) {
				if (o) {
					if(o.exId){
						Ext.MessageBox.alert('提示', o.exDesc||'操作错误！', function(va) {
									if(va=='yes'){
...
IsmpHB.admin.modPwdDlg = Ext.extend(Ext.Window, {
	title : '修改密码窗口',
	layout : 'fit',
	modal : true,
	width : 350,
	...
	constructor : function(config) {
		var config = config || {};
		config.items = this.items || [];
		this.configForm = new IsmpHB.admin.ModPwdForm({});
		config.items.push(this.configForm);
		IsmpHB.admin.modPwdDlg.superclass.constructor.apply(this, arguments);
...
IsmpHB.admin.AdminForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 70,
	labelAlign : 'right',
	bodyStyle : 'padding:5px 5px 5px 5px;',
	border : false,
...
//		config.items.push(this.typeCombo);
		...
		IsmpHB.admin.AdminForm.superclass.constructor.apply(this, arguments);	
		this.addBtn.on('click', function() {
			this.toadd();
		}, this);
...
//		this.typeCombo.reset();
	...
	isValid : function() {
		return this.account.isValid() && this.pwd.isValid() && this.node.isValid() && this.typeCombo.isValid()
		&&this.account.getValue().length>0&&this.name.isValid()&&this.name.getValue().length>0&&this.role.submitValue!='';
	},
...
IsmpHB.admin.updateForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 70,
	labelAlign : 'right',
	bodyStyle : 'padding:5px 5px 5px 5px;',
	border : false,
...
//		invalidText : '不能全为空格',
	...
	constructor : function(config) {
		...
		config.buttons.push(this.cancelBtn);
		IsmpHB.admin.updateForm.superclass.constructor.apply(this, arguments);
	...
	isValid : function() {
		return this.account.isValid() && this.name.isValid() && this.node.isValid() && this.desc.isValid()&&this.role.submitValue!='';
	},
...
IsmpHB.admin.updateDlg = Ext.extend(Ext.Window, {
	title : '更新管理员窗口',
	layout : 'fit',
	modal : true,
	width : 350,
	...
	constructor : function(config) {
		var config = config || {};
		config.items = this.items || [];
		this.configForm = new IsmpHB.admin.updateForm({});
		config.items.push(this.configForm);
		IsmpHB.admin.updateDlg.superclass.constructor.apply(this, arguments);
...
IsmpHB.admin.AdminDlg = Ext.extend(Ext.Window, {
	title : '新增管理员窗口',
	layout : 'fit',
	modal : true,
	width : 420,
	...
	constructor : function(config) {
		var config = config || {};
		config.items = this.items || [];
		this.configForm = new IsmpHB.admin.AdminForm({});
		config.items.push(this.configForm);
		IsmpHB.admin.AdminDlg.superclass.constructor.apply(this, arguments);
...
IsmpHB.admin.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '系统账号管理',
	autoScroll : true,
	...
	constructor : function(config) {
		this.adDlg = new IsmpHB.admin.AdminDlg({});
		this.updateDlg = new IsmpHB.admin.updateDlg({});
		this.modPwdDlg = new IsmpHB.admin.modPwdDlg({});
		var config = config || {};
		config.tbar = config.tbar || [];
		var a = IsmpHB.common.getPermission('4-1');
		...
		if(IsmpHB.common.isHasPermission(a,12))
...
//			this.adDlg.configForm.resetForm();
		...
		this.modPwdBtn.on('click',function(){
			...
			if (rs == null) {
				...
				this.modPwdDlg.title="修改"+rs.data.account+"的密码";
				this.modPwdDlg.configForm.account.setValue(rs.data.account);		
				this.modPwdDlg.show();
			}
...
IsmpHB.admin.Information = Ext.extend(Ext.grid.GridPanel, {
	title : '账号安全审计',
	autoScroll : true,
...
//		disabled : true,
	...
	constructor : function(config) {
		var config = config || {};
		config.tbar = config.tbar || [];
		var a = IsmpHB.common.getPermission('4-2');
...
//		}, 
		...
		this.refeshBtn.on('click', function() {
			...
			this.getStore().setBaseParam('loginName', loginName);
			this.getStore().load({
				params: {
					login : login,
