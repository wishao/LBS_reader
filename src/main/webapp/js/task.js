...
Ext.namespace('IsmpHB', 'IsmpHB.task');
...
var qstatusTypeData = [ ['-1','所有'],[ '0', '导入中' ], [ '1', '导入完成' ], [ '2', '任务启动' ],['3','进行中'],['4','完成'],['5','失败'] ];
var qjobPlanData = [['-1','所有'],[ '0', '定时' ], [ '1', '立即执行' ]];
var taskListStatusData = [['-1','所有'],[ '0', '未开始' ],[ '1', '进行中' ], [ '2', '成功' ],['3','失败']];

global_task_no = -1;

IsmpHB.task.ItemForm = Ext.extend(Ext.form.FormPanel,{
	labelWidth : 90,
	labelAlign : 'right',
	border : false,
	fileUpload : true,
...
//	}),
	...
	constructor : function(config) {
		...
		var jobInfo = {
            ...
            border : true,
            autoHeight: true,
            items : [this.taskNoField,this.jobTypeField,this.levelField,this.jobPlan,this.planDate]
		};
		var tacticInfo = {
			xtype: 'fieldset',
            title: '数据重试策略',
...
IsmpHB.task.ItemDlg = Ext.extend(Ext.Window, {
	title : '新增任务',
	layout : 'fit',
	modal : true,
	width : 550,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		this.configForm = new IsmpHB.task.ItemForm({});
...
IsmpHB.task.TaskList = Ext.extend(Ext.grid.GridPanel,{
	autoScroll : true,
...
//		this.setBtnVisible(this.statusHidden.getValue());
		...
		var itemsPanel = new Ext.Panel({
			...
			items : [{
						xtype : 'toolbar',
						border : false,
						items : ['数据总量：', this.totalNumField/*, '已处理：',
								this.finishNumField*/]
					},{
						xtype : 'toolbar',
						border : false,
						items :['成功数据总量：', this.sucessNumField,this.impSucBtn,this.downExportForm]
					},{
						xtype : 'toolbar',
						border : false,
		...
		this.cm = new Ext.grid.ColumnModel([ {
			...
			dataIndex : 'status',
			width : 100,
			renderer : IsmpHB.renderer.TASKLIST_STATUS
		}, {
			header : '处理结果',
			align : 'left',
			menuDisabled : true,
		...
		IsmpHB.task.TaskList.superclass.constructor.apply(this, arguments);
	...
	setBtnVisible : function(value){
		if(value == IsmpHB.data.TASK_STATUS.FINISH){
			this.impSucBtn.show();
...
//		this.finishNumField.setValue();
	...
	exportData : function(value){
		if(this.statusHidden.getValue()!=IsmpHB.data.TASK_STATUS.FINISH){
			Ext.MessageBox.alert('Excel文件导出提示', '任务未完成，不允许导出！', null,
						this);
			return;
		}
...
IsmpHB.task.TaskListWin = Ext.extend(Ext.Window, {
	title : '任务详情',
	layout : 'fit',
	modal : true,
	maximizable : true,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items||[];
		this.gridPanel = new IsmpHB.task.TaskList({});
		config.items.push(this.gridPanel);
		IsmpHB.task.TaskListWin.superclass.constructor.apply(this, arguments);
...
IsmpHB.task.Task = Ext.extend(Ext.grid.GridPanel,{
//	loadMask : {
//		msg : '正在加载数据，请稍候...'
//	},
	title : '任务导入',
...
//	}),
	...
	constructor : function(config) {
		...
		config.bbar = config.bbar || [];
//		config.items = config.items || [];
		this.createTaskDlg = new IsmpHB.task.ItemDlg({});
		this.pagingbar = new Ext.PagingToolbar({
			pageSize : 15,
			store : this.getStore(),
			displayInfo : true,
		...
		config.bbar = this.pagingbar;
		var a = IsmpHB.common.getPermission('2-4');
		var arr = ['执行时间:',this.startTimeField,'创建者:',this.operatorText,'->'];
		...
		var itemPanel = new Ext.Panel({
			...
			items : [{
						xtype : 'toolbar',
						border : false,
						items : ['任务编号:', this.taskNoField,'任务类型:',this.jobTypeField, '任务等级:',
								this.taskLevelCob, '任务计划:', this.taskPlanCob, '状态:',
								this.statusField]
					},{
						xtype : 'toolbar',
						border : false,
		...
		this.cm = new Ext.grid.ColumnModel([ this.sm, {
			...
			dataIndex : 'status',
			width : 100,
			renderer : IsmpHB.renderer.TASK_STATUS
		}, {
			header : '任务计划',
			align : 'left',
			menuDisabled : true,
			dataIndex : 'task_plan',
			width : 80,
			renderer : IsmpHB.renderer.TASK_PLAN
		}, {
			header : '任务类型',
			align : 'left',
			menuDisabled : true,
			dataIndex : 'task_type',
			width : 80,
			renderer : IsmpHB.renderer.TASK_TYPE
		}, {
			header : '任务优先级',
			align : 'left',
			menuDisabled : true,
			dataIndex : 'level',
			width : 80,
			renderer : IsmpHB.renderer.TASK_LEVEL
		}, {
			header : '任务总量',
			align : 'left',
			menuDisabled : true,
		...
		IsmpHB.task.Task.superclass.constructor.apply(this, arguments);

		this.refeshBtn.on('click', function() {
			var timevalue = null;
			if(this.startTimeField.getValue()){
				timevalue = this.startTimeField.getValue().format(Date.patterns.ISO8601Short)
			}
			this.getStore().baseParams = {
				start : 0,
					limit : this.pagingbar.pageSize,
					...
					operator_name : this.operatorText.getValue()|| null
			};
			this.getStore().load({
				params : {
					start : 0,
...
//					result_code : resultField,
		...
		this.checkBtn.on('click', function() {
			...
			var win = new IsmpHB.task.TaskListWin({});
