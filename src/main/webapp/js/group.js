 ...
 * @author johnny0086
 */
Ext.namespace('LBSReader', 'LBSReader.group');

LBSReader.group.GroupProductRecord = Ext.data.Record.create([{
			name : 'name',
			mapping : 'name'
		}, {
			name : 'maxSelected',
...
LBSReader.group.GroupProductGrid = Ext.extend(Ext.grid.GridPanel, {
			autoScroll : true,
			height : 240,
			...
			constructor : function(config) {
				...
				LBSReader.group.GroupProductGrid.superclass.constructor.apply(
						this, arguments);
...
LBSReader.group.PackageGroupForm = Ext.extend(Ext.form.FormPanel, {
			labelWidth : 90,
			labelAlign : 'right',
			border : false,
			...
			grid : new LBSReader.group.GroupProductGrid({}),
			...
			constructor : function(config) {
				...
				LBSReader.group.PackageGroupForm.superclass.constructor.apply(
						this, arguments);
			...
			commitAdd : function() {
				...
				var record = new LBSReader.group.GroupProductRecord({
...
LBSReader.group.PackageGroupDlg = Ext.extend(Ext.Window, {
			title : '产品组配置',
			layout : 'fit',
			modal : true,
			width : 380,
			...
			constructor : function(config) {
				config = config || {};
				config.items = config.items || [];
				this.configForm = new LBSReader.group.PackageGroupForm({});
				config.items.push(this.configForm);
				LBSReader.group.PackageGroupDlg.superclass.constructor.apply(
						this, arguments);
...
LBSReader.group.PackageGroupGrid = Ext.extend(Ext.grid.GridPanel, {
			autoScroll : true,
			height : 165,
			...
			constructor : function(config) {
				this.dlg = new LBSReader.group.PackageGroupDlg({});
				config = config || {};
				config.bbar = [];
				config.bbar.push(this.addBtn);
				...
				LBSReader.group.PackageGroupGrid.superclass.constructor.apply(
						this, arguments);
