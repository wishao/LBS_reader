 ...
 * @author johnny0086
 */
Ext.namespace('LBSReader', 'LBSReader.ppkg');

LBSReader.ppkg.ItemForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 80,
	labelAlign : 'right',
	border : false,
	autoScroll : true,
	...
	constructor : function(config) {
		this.isEDIT = false;
		this.grid = new LBSReader.group.PackageGroupGrid({});
		config = config || {};
		...
		LBSReader.ppkg.ItemForm.superclass.constructor.apply(this, arguments);
	...
	commitAdd : function() {
		...
		var req = {
			url : LBSReader.req.HBPACKAGE_COMPLEX_ADD,
			params : {
	...
	commitEdit : function() {
		...
		var req = {
			url : LBSReader.req.HBPACKAGE_COMPLEX_UPDATE,
			params : {
				timestamp : new Date().valueOf(),
...
LBSReader.ppkg.ItemDlg = Ext.extend(Ext.Window, {
			title : '复杂产品包配置',
			layout : 'fit',
			modal : true,
			width : 400,
			...
			constructor : function(config) {
				config = config || {};
				config.items = config.items || [];
				this.configForm = new LBSReader.ppkg.ItemForm({});
				config.items.push(this.configForm);
				LBSReader.ppkg.ItemDlg.superclass.constructor.apply(this,
						arguments);
...
LBSReader.ppkg.DataGrid = Ext.extend(Ext.grid.GridPanel, {
			title : '复杂产品包配置',
			autoScroll : true,
			...
			constructor : function(config) {
				this.dlg = new LBSReader.ppkg.ItemDlg({});
				...
				this.cm = new Ext.grid.ColumnModel([this.sm, {
							...
							menuDisabled : true,
							dataIndex : 'groups',
							renderer : LBSReader.renderer.PACKAGE_PRODUCT,
							width : 200
						}, {
							header : '计费编码',
							align : 'center',
				...
				LBSReader.ppkg.DataGrid.superclass.constructor.apply(this,
						arguments);
			...
			removeItems : function() {
				...
				if (null != r && null != r.data) {
					var req = {
						url : LBSReader.req.HBPACKAGE_COMPLEX_REMOVE,
						params : {
							timestamp : new Date().valueOf(),
							id : r.data.id
						},
