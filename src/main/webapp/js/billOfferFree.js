 ...
 */
Ext.namespace('IsmpHB', 'IsmpHB.offerFree');

var freeTypeData = [ [ '0', '按计费号码免单' ], [ '1', '按套餐代码免单' ], [ '2', '号码全业务免单' ]];

IsmpHB.offerFree.offerChooseGridDlg = Ext.extend(Ext.Window, {
	title : '报盘话单',
	layout : 'fit',
	modal : true,
	width : 950,
	...
	constructor : function(config) {
		...
		this.configGrid = new IsmpHB.offerFree.offerDataGrid({});
		config.items.push(this.configGrid);
		IsmpHB.offerFree.offerChooseGridDlg.superclass.constructor.apply(this,
				arguments);

		this.on('show', function() {			
			//Ext.MessageBox.alert('提示', this.configGrid.telHidden.getValue(),null, this);
			this.configGrid.searchData();
		}, this);
	},
	setValues : function(tel,busiCode, billCode,sType,accDuration,tableIndex) {
...
IsmpHB.offerFree.offerDataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '话单报盘',
	autoScroll : true,
	...
	constructor : function(config) {
		...
		IsmpHB.offerFree.offerDataGrid.superclass.constructor.apply(this,
				arguments);
	},
...
IsmpHB.offerFree.DataGrid = Ext.extend(Ext.grid.GridPanel,
		{
			title : '免单号码查询',
			autoScroll : true,
			...
			statusCombo : new Ext.form.ComboBox({
				...
				mode : 'local',
				triggerAction : 'all',
				store : IsmpHB.store.COST_FREE_STATUE,
				width : 80
			}),
			...
			constructor : function(config) {
				this.offerChooseDlg = new IsmpHB.offerFree.offerChooseGridDlg(
						{});
				this.createFreeDlg = new IsmpHB.offerFree.ItemDlg({});
				config = config || {};
				config.tbar = config.tbar || [];
					...
					items : [
							{
								xtype : 'toolbar',
								border : false,
								items : [ '电话:', this.telText,'套餐代码：',this.billCodeText,
										'计费代码：',this.busiCodeText,'批次号：',this.taskNoText, '地市：',
										this.cityCombo, '帐期：',this.accDurationText ,'免单状态：',this.statusCombo
										]
							},
							{
								xtype : 'toolbar',
								...
								items : [ this.searchBtn, this.resetBtn, '->',
										 this.hasFreeBtn,this.toFreeBtn,this.addCostFreeBtn ]
							} ]
				}));
				...
				this.cm = new Ext.grid.ColumnModel([ this.sm, {
					...
					dataIndex : 'sType',
					width : 100,
					renderer : IsmpHB.renderer.BILL_COST_FREE_STYPE
				}, {
					header : '免单状态',
					align : 'left',
					menuDisabled : true,
					dataIndex : 'status',
					width : 100,
					renderer : IsmpHB.renderer.BILL_COST_FREE_OP_STATUS
				}, {
					header : '批次号',
					align : 'left',
					menuDisabled : true,
				...
				IsmpHB.offerFree.DataGrid.superclass.constructor.apply(this,
						arguments);
				...
				this.hasFreeBtn.on('click',
						function() {
							var rs = this.getSelectionModel().getSelections();
							if (rs.length == 0) {
								Ext.MessageBox.alert('提示', '请选择要查看已免单的记录！',
										null, this);
								return;
							}							
							if(rs[0].data.productSpecCode=='ALL'){
								this.offerChooseDlg.setValues(rs[0].data.tel,'','','',rs[0].data.accDuration,'1');
							}else if(rs[0].data.busiCode!=''){
								this.offerChooseDlg.setValues(rs[0].data.tel,rs[0].data.busiCode,'','',rs[0].data.accDuration,'1');
							}else{
								this.offerChooseDlg.setValues(rs[0].data.tel,'',
										rs[0].data.billCode,'',rs[0].data.accDuration,'1');
							}						
							this.offerChooseDlg.show();
						}, this);
				this.toFreeBtn.on('click',
						function() {
							var rs = this.getSelectionModel().getSelections();
							if (rs.length == 0) {
								Ext.MessageBox.alert('提示', '请选择要查看已免单的记录！',
							...
							if(rs[0].data.productSpecCode=='ALL'){
								this.offerChooseDlg.setValues(rs[0].data.tel,'','','',rs[0].data.accDuration,'0');
							}else if(rs[0].data.busiCode!=''){
								this.offerChooseDlg.setValues(rs[0].data.tel,rs[0].data.busiCode,'','',rs[0].data.accDuration,'0');
							}else{
								this.offerChooseDlg.setValues(rs[0].data.tel,'',
										rs[0].data.billCode,'',rs[0].data.accDuration,'0');
							}							
							this.offerChooseDlg.show();
					}, this);
			},
...
IsmpHB.offerFree.ItemDlg = Ext.extend(Ext.Window, {
	title : '导入免单',
	layout : 'fit',
	modal : true,
	width : 550,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		this.configForm = new IsmpHB.offerFree.ItemForm({});
		config.items.push(this.configForm);
		IsmpHB.task.ItemDlg.superclass.constructor.apply(this, arguments);
...
IsmpHB.offerFree.ItemForm = Ext.extend(Ext.form.FormPanel,{
	labelWidth : 90,
	labelAlign : 'right',
	border : false,
	fileUpload : true,
	...
	constructor : function(config) {
		...
		var jobInfo = {
            ...
            border : true,
            autoHeight: true,
            items : [this.taskNoField ,this.accDurationField,this.freeTypeField]
		};
		var tacticInfo = {
			xtype: 'fieldset',
            title: '数据重试策略',
		...
		var dataInfo = {
            ...
            border : true,
            autoHeight: true,
            items : [this.linkField,this.remField,this.fileSelectField,this.memoField]
		};
		this.remField.hide();
...
//		config.items.push(tacticInfo);
		...
		IsmpHB.task.ItemForm.superclass.constructor.apply(this,arguments);
	...
	addTask : function(){
		...
		var req = {
			url : IsmpHB.req.BILL_COST_FREE_ADD,
			params : params,
			method : 'post',
			scope : this,
			waitMsg : '正在上传文件',
