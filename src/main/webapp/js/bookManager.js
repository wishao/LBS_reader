...
LBSReader.bookManager.ItemForm = Ext.extend(Ext.form.FormPanel, {
	...
	constructor : function(config) {
		...
		config.items.push([ {
			...
			items : [ this.idHidden, this.nameField, this.authorField,
					this.contentField, this.recommendField, this.coverField,
					this.catalogField, this.statusCombo ]
		} ]);

		config.buttons = config.buttons || [];
		config.buttons.push(this.bookCommitBtn);
		config.buttons.push(this.cancelBtn);
		LBSReader.bookManager.ItemForm.superclass.constructor.apply(this,
				arguments);
	...
	isValid : function() {
		return this.nameField.isValid() && this.authorField.isValid()
				&& this.contentField.isValid() && this.recommendField.isValid()
				&& this.coverField.isValid() && this.catalogField.isValid()
...
LBSReader.bookManager.ItemDlg = Ext.extend(Ext.Window, {
	...
	constructor : function(config) {
		...
		LBSReader.bookManager.ItemDlg.superclass.constructor.apply(this,
				arguments);
	},
	toAdd : function() {
...
LBSReader.bookManager.coverDlg = Ext.extend(Ext.Window, {
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		LBSReader.bookManager.coverDlg.superclass.constructor.apply(this,
				arguments);
...
LBSReader.bookManager.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	...
	constructor : function(config) {
		...
		LBSReader.bookManager.DataGrid.superclass.constructor.apply(this,
				arguments);
		...
		this.remvBtn.on('click', function() {
			...
			Ext.MessageBox.confirm("提示", "确认要把所选记录删除(不可逆)吗？", function(id) {
				if (id == "yes") {
					this.removeItems();
				}
			}, this);
