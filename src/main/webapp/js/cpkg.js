 ...
 * @author johnny0086
 */
Ext.namespace('IsmpHB', 'IsmpHB.cpkg');

IsmpHB.cpkg.ProductGrid = Ext.extend(Ext.grid.GridPanel, {
	autoScroll : true,
	...
	constructor : function(config) {
		...
		IsmpHB.cpkg.ProductGrid.superclass.constructor.apply(this, arguments);

		this.addBtn.on('click', function() {
			var rs = this.getSelectionModel().getSelections();
			for ( var i = 0; i < Ext.StoreMgr.get('groupitem').getCount(); i++) {
...
IsmpHB.cpkg.ProductDlg = Ext.extend(Ext.Window, {
	title : '选择产品',
	layout : 'fit',
	modal : true,
	width : 440,
	...
	constructor : function(config) {
		...
		this.grid = new IsmpHB.cpkg.ProductGrid({});
		config.items.push(this.grid);
		IsmpHB.cpkg.ProductDlg.superclass.constructor.apply(this, arguments);
...
IsmpHB.cpkg.GroupItemGrid = Ext.extend(Ext.grid.GridPanel, {
	autoScroll : true,
	height : 150,
	...
	constructor : function(config) {
		...
		IsmpHB.cpkg.GroupItemGrid.superclass.constructor.apply(this, arguments);
...
IsmpHB.cpkg.ItemForm = Ext.extend(Ext.form.FormPanel, {
	labelWidth : 90,
	labelAlign : 'right',
	border : false,
	autoScroll : true,
	...
	beginRuleCmb : new Ext.form.ComboBox({
		...
		store : new Ext.data.ArrayStore({
			fields : ['mode','name'],
			data : [[1,'当月生效，收取全月费用'],[2,'每月1-15日开户，收全价费用，16日后开户收半月费用'],[3,'每月1-22日开户收全月费用，22日后开户，当月免费'],
				[4,'收取全年费用'],[5,'收取全月费用'],[6,'按天计费'],[7,'1-22日收全月费用，23日后免费'],[99,'其他']]
		}),
		value : 2
	}),
	endPreCmb : new Ext.form.ComboBox({
		...
		store : new Ext.data.ArrayStore({
			fields : ['mode','name'],
			data : [[1,'当月生效，不计费'],[2,'订购当月按天收费'],[3,'每月1-9日注销，当月不计费，10日至月末注销，收当月费用']
				,[4,'次月生效'],[99,'其他']]
		}),
		value : 3
	}),
	endAfterCmb : new Ext.form.ComboBox({
		...
		store : new Ext.data.ArrayStore({
			fields : ['mode','name'],
			data : [[1,'当月生效，不计费'],[2,'次月生效，当月按正常收费'],[3,'每月1-15日注销，当月不计费，16日至月末注销，收当月半价费用'],
				[4,'到期自动失效'],[5,'1-15日免费，16日后半价'],[99,'其他']]
		}),
		value : 3
	}),

	grid : new IsmpHB.cpkg.GroupItemGrid({}),
	...
	constructor : function(config) {
		...
		config.items.push({
			...
			layout: 'form',
			bodyBorder : false,
			items :[this.idHidden,this.nameField,this.seqNumField,this.smaxField,
				this.billCombo,this.chargingCodeField,this.chargingCycleCmb,this.chargingDescTex,
				this.effectModeCmb,this.withDrawModeCmb,this.trialTypeCmb,this.trialTermField]
		},{
			columnWidth: .5,
			bodyBorder : false,
			layout: 'form',
			items : [this.useFlagCmb,this.beginRuleCmb,this.endPreCmb,this.endAfterCmb,this.cMaxField,this.fieldSet]
		});
		config.buttons = config.buttons || [];
		config.buttons.push(this.commitBtn);
		IsmpHB.cpkg.ItemForm.superclass.constructor.apply(this, arguments);
	...
	isValid : function() {
		return this.nameField.isValid() && this.seqNumField.isValid() && this.smaxField.isValid() && this.cMaxField.isValid()
	...
	commitAdd : function() {
		...
		if (!this.grid.isValid(this.cMaxField.getValue())) {
			var errorMsg = '最大可选项为' + this.cMaxField.getValue() + '项, ' + '您的默认项已经选择了' + this.grid.getStore().query('selectedFlag', 1).getCount() + '项！';
			Ext.Msg.show({
				title : '融合产品包配置',
				msg : errorMsg,
				buttons : Ext.Msg.OK,
		...
		var req = {
			url : IsmpHB.req.HBPACKAGE_MULTI_ADD,
			params : {
				timestamp : new Date().valueOf(),
	...
	commitEdit : function() {
		...
		var req = {
			url : IsmpHB.req.HBPACKAGE_MULTI_UPDATE,
			params : {
				timestamp : new Date().valueOf(),
...
IsmpHB.cpkg.ItemDlg = Ext.extend(Ext.Window, {
	title : '融合产品包配置',
	layout : 'fit',
	modal : true,
	width : 740,
	...
	constructor : function(config) {
		config = config || {};
		config.items = config.items || [];
		this.configForm = new IsmpHB.cpkg.ItemForm({});
		config.items.push(this.configForm);
		IsmpHB.cpkg.ItemDlg.superclass.constructor.apply(this, arguments);
	},
	toAdd : function() {
...
IsmpHB.cpkg.DataGrid = Ext.extend(Ext.grid.GridPanel, {
	title : '融合产品包配置',
	autoScroll : true,
	...
	constructor : function(config) {
		this.dlg = new IsmpHB.cpkg.ItemDlg({});
		this.pagingbar = new Ext.PagingToolbar({
			pageSize : 15,
			store : this.getStore(),
			displayInfo : true,
		...
		this.cm = new Ext.grid.ColumnModel([ this.sm, {
			...
			menuDisabled : true,
			dataIndex : 'multi',
			renderer : IsmpHB.renderer.PACKAGE_MULTI_PRODUCT,
			width : 200
		},{
			header : '计费编码',
			align : 'left',
		...
		IsmpHB.cpkg.DataGrid.superclass.constructor.apply(this, arguments);
	...
	loadItems : function() {
		if(null == this.nameField.getValue()||''==this.nameField.getValue().trim()){
	...
	removeItems : function() {
		...
		if (null != r && null != r.data) {
			var req = {
				url : IsmpHB.req.HBPACKAGE_MULTI_REMOVE,
				params : {
					timestamp : new Date().valueOf(),
					id : r.data.id,
					packageType: '1'
